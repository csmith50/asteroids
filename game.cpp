...
void Game::collisionDetect()
{
	for (int i = 0; i < bullets.size(); i++)
	{
		for (int j = 0; j < rocks.size(); j++) // loop through all of the rocks 
		{
			if (bullets[i].isAlive())
			{
				if (rocks[i] != NULL && (rocks[j]->isAlive())) // if the current rock is alive
				{
					if (getClosestDistance(*rocks[j], bullets[i]) < CLOSE_ENOUGH) // we have a hit
					{
						cout << "hit!" << endl;
						bullets[i].kill();
						rocks[j]->hit(rocks, rocks[j]->getVelocity(), rocks[j]->getPoint());
					}
				}
			}
		}
